/**
 * SoftIXX MAIN JQUERY JS <br>
 * Require jQuery, Bootstrap, FontAwesome and other plugins (folder ../plugins)
 * @version 1.0.0
 * @author Maikel Guerra Ferrer - mguerraferrer@gmail.com
 * 
 */

const $_BOOTSTRAP_VERSION = 5;
const $_LOCALE = 'es_MX';
const $_LOCALE_OPTION = 'es';
const $_PAGINATE_SIZE = 10;
const $_FORM_TYPES_ARRAY = ['text', 'email', 'number', 'hidden', 'select-one', 'select-multiple', 'textarea', 'checkbox', 'radio', 'password', 'file'];
...
const $_LOADING_MESSAGE = 'Cargando información...';
const $_PROCESSING_MESSAGE = 'Procesando solicitud...';
const $_SORTING_MESSAGE = 'Ordenando registros...';
const $_PAGING_MESSAGE = 'Paginando registros...';
const $_REQUIRED_MESSAGE = 'El valor de este campo es requerido';
const $_CONFIRMATION_TEXT = '¿Está seguro que desea eliminar el elemento seleccionado?';
const $_EMPTY_PANEL_MESSAGE = 'No existe información a mostrar';
const $_EMPTY_SEARCH_PANEL_MESSAGE = '';
const $_ERROR_MESSAGE = 'Ha ocurrido un error interno en el servidor. La solicitud ha sido cancelada';
const $_ERROR_400 = 'Solicitud incorrecta';
const $_ERROR_400_DESCRIPTION = 'La solicitud enviada es incorrecta o no existe';
const $_ERROR_INFO = 'Información del error';
const $_PAGE_HEAD_SELECTOR = 'page-head';
const $_PAGE_LOADER_SELECTOR = 'page-loader';
const $_PAGE_CONTENT_SELECTOR = 'page-content';
const $_PAGE_VALID_CONTENT_SELECTOR = 'page-valid-content';
const $_PAGE_MESSAGE_SELECTOR = 'page-message';
const $_PAGE_MESSAGE_FRAGMENT = 'page-message-fragment';
const $_PAGE_ALERT_SELECTOR = 'page-alert';
const $_INFO_CLASS = 'alert alert-info';
const $_INFO_CLASS_DISMISSIBLE = 'alert alert-info alert-dismissible';
const $_SUCCESS_CLASS = 'alert alert-mint';
const $_SUCCESS_CLASS_DISMISSIBLE = 'alert alert-success alert-dismissible';
const $_WARNING_CLASS = 'alert alert-warning';
const $_WARNING_CLASS_DISMISSIBLE = 'alert alert-warning alert-dismissible';
const $_DANGER_CLASS = 'alert alert-danger';
const $_DANGER_CLASS_DISMISSIBLE = 'alert alert-danger alert-dismissible';
const $_ALERT_INFO_CLASS = 'alert-info';
const $_ALERT_INFO_ICON_CLASS = 'fad fa-info-circle';
const $_ALERT_SUCCESS_CLASS = 'alert-mint';
const $_ALERT_SUCCESS_ICON_CLASS = 'fas fa-check-circle';
const $_ALERT_WARNING_CLASS = 'alert-warning';
const $_ALERT_WARNING_ICON_CLASS = 'fas fa-exclamation-triangle';
const $_ALERT_DANGER_CLASS = 'alert-danger';
const $_ALERT_DANGER_ICON_CLASS = 'fas fa-ban';
const $_PANEL_SEARCH_EMPTY_SELECTOR = 'panel-search-result-empty-message';

const $_LI_PAGE_SIZE_MAP = new Map();
$_LI_PAGE_SIZE_MAP.set(10, 'li-pag-size-10');
$_LI_PAGE_SIZE_MAP.set(20, 'li-pag-size-20'); 
$_LI_PAGE_SIZE_MAP.set(30, 'li-pag-size-30'); 
$_LI_PAGE_SIZE_MAP.set(40, 'li-pag-size-40'); 
$_LI_PAGE_SIZE_MAP.set(50, 'li-pag-size-50');
...
const jsOptions = {
    locale: $_LOCALE,
    localeOption: $_LOCALE_OPTION,
    bootstrapVersion: $_BOOTSTRAP_VERSION,
    paginateSize: $_PAGINATE_SIZE,
    loadingMessage: $_LOADING_MESSAGE,
    processingMessage: $_PROCESSING_MESSAGE,
    sortingMessage: $_SORTING_MESSAGE,
    pagingMessage: $_PAGING_MESSAGE,
    requiredMessage: $_REQUIRED_MESSAGE,
    confirmationText: $_CONFIRMATION_TEXT,
    emptyPanelMessage: '',
    emptySearchPanelMessage: '',
    errorMessage: $_ERROR_MESSAGE,
    error400: $_ERROR_400,
    error400Description: $_ERROR_400_DESCRIPTION,
    errorInfo: $_ERROR_INFO,
	pageMessageConditional: true,
    pageHeadSelector: $_PAGE_HEAD_SELECTOR,
    pageLoaderSelector: $_PAGE_LOADER_SELECTOR,
    pageContentSelector: $_PAGE_CONTENT_SELECTOR,
    pageValidContentSelector: $_PAGE_VALID_CONTENT_SELECTOR,
    pageMessageSelector: $_PAGE_MESSAGE_SELECTOR,
    pageMessageFragment: $_PAGE_MESSAGE_FRAGMENT,
    pageAlertSelector: $_PAGE_ALERT_SELECTOR,
	infoClass: $_INFO_CLASS,
    infoClassDismissible: $_INFO_CLASS_DISMISSIBLE,
    successClass: $_SUCCESS_CLASS,
    successClassDismissible: $_SUCCESS_CLASS_DISMISSIBLE,
    warningClass: $_WARNING_CLASS,
    warningClassDismissible: $_WARNING_CLASS_DISMISSIBLE,
    dangerClass: $_DANGER_CLASS,
    dangerClassDismissible: $_DANGER_CLASS_DISMISSIBLE,
    alertInfoClass: $_ALERT_INFO_CLASS,
    alertInfoIconClass: $_ALERT_INFO_ICON_CLASS,
    alertSuccessClass: $_ALERT_SUCCESS_CLASS,
    alertSuccessIconClass: $_ALERT_SUCCESS_ICON_CLASS,
    alertWarningClass: $_ALERT_WARNING_CLASS,
    alertWarningIconClass: $_ALERT_WARNING_ICON_CLASS,
    alertDangerClass: $_ALERT_DANGER_CLASS,
    alertDangerIconClass: $_ALERT_DANGER_ICON_CLASS,
    serverContext: null,
...
const $_AJAX_RESPONSE = {
	...
	expires: 0,				//if session expires
	invalidSession: false, 	//is session is invalid
	error: null				//contains the error of te request
};
...
const $_FIELD = {
    ...
    scInstance({field, message, scroll = true, container = null}) {
        this.field = field;
        this.message = message;
        this.container = container;
        this.scrollToElement = scroll;
        this.fillData();
        return this;
    },
    mcInstance(field, message = null, messageContainer = null) {
        this.field = field;
        this.message = message;
    ...
    fillData() {
        this.div = (this.container == null) ? 'div-' + this.field : this.container;
        this.label = 'label-' + this.field;
        this.small = (this.messageContainer == null) ? 'small-' + this.field : this.messageContainer;
...
function pageLoaded() {
    ...
    if (exists(jsOptions.pageLoaderSelector)) {
        elementRemove(jsOptions.pageLoaderSelector);
    }
...
const errorResponse = (response) => {
    ...
    if (response.licenseExpired || response.hiddenPageContent) {
        elementEmpty('page-valid-content');
    }
...
function pageMsgHide() {
    const pageMessageSelector = jsOptions.pageMessageSelector;
    if (exists(pageMessageSelector)) {
        elementClear('global-message');
        elementClear('global-message-text');
...
function panelMsg(notificationBaseObj) {
    ...
    if(exists(div)) {
        ...
        if (hasClass(divAlert, jsOptions.alertDangerClass)) {
            elementClassAdd(iId, jsOptions.alertDangerIconClass);
        } else if (hasClass(divAlert, jsOptions.alertWarningClass)) {
            elementClassAdd(iId, jsOptions.alertWarningIconClass);
        } else if (hasClass(divAlert, jsOptions.alertInfoClass)) {
            elementClassAdd(iId, jsOptions.alertInfoIconClass);
        } else if (hasClass(divAlert, jsOptions.alertSuccessClass)) {
            elementClassAdd(iId, jsOptions.alertSuccessIconClass);
        }
...
function evaluatePanelCollapsable(panelId, error, panelHide = false) {
    ...
    if (error) {
        elementClassChange(panelId, 'panel-bordered-primary', 'panel-bordered panel-danger');

        elementClassRemove(panelHeading, 'panel-heading-transparent');
        elementClassRemove(panelHeading, 'collapsed');
        elementAttr(panelHeading, 'aria-expanded', true);

        elementClassAdd(panelBody, 'collapse in');
        elementAttr(panelBody, 'aria-expanded', true);
        elementPropertyRemove(panelBody, 'height');
    } else {
        elementClassRemove(panelId, 'panel-bordered panel-danger');
        elementClassAdd(panelId, 'panel-bordered-primary');
        elementClassAdd(panelHeading, 'panel-heading-transparent');
...
function panelCollapsableMessageShow(panelId, pMessage, pClass = null) {
    ...
    if (hasClass(divAlert, 'alert-danger')) {
        elementClassAdd(iId, 'fas fa-ban');
    } else if (hasClass(divAlert, 'alert-warning')) {
        elementClassAdd(iId, 'fas fa-exclamation-triangle');
    } else if (hasClass(divAlert, 'alert-info')) {
        elementClassAdd(iId, 'fad fa-info-circle');
    } else if (hasClass(divAlert, 'alert-success')) {
        elementClassAdd(iId, 'fas fa-check-circle');
    }
...
function notificationShow({container, message, styleClass = null, dismiss = null, fadeOut = null}) {
    ...
    if (hasClass('global-message-alert', 'alert-danger')) {
        elementClassAdd('i-global-message', 'fas fa-ban');
    } else if (hasClass('global-message-alert', 'alert-warning')) {
        elementClassAdd('i-global-message', 'fas fa-exclamation-triangle');
    } else if (hasClass('global-message-alert', 'alert-info')) {
        elementClassAdd('i-global-message', 'fad fa-info-circle');
    } else if (hasClass('global-message-alert', 'alert-success')) {
        elementClassAdd('i-global-message', 'fas fa-check-circle');
    }
        ...
        if (search.panel.length) {
            search.panel.each(function () {
                ...
                $(this).children().each(function () {
                    ...
                    elementClassAdd(panelCollapse, collapse);
...
(function ($) {
    ...
    $(document).on('doc.ready', function () {
        $('body').on('click', '[data-sort]', function () {
            ...
            $('#' + target).children().each(function () {
                if ($(this).prop('id')) {
                    ...
                    if ($_SORT_DIRECTION == 'asc') {
                        ...
                        elementClassChange(child, 'fa-sort-amount-down', 'fa-sort-amount-down');
                    } else if ($_SORT_DIRECTION == 'desc') {
                        $('#' + target).attr("data-sort-direction", "asc");
                        elementClassChange(child, 'fa-sort-amount-up', 'fa-sort-amount-up');
                    }
                    if(jsOptions.showLog) {
            ...
            elementClassAdd(target, 'data-sort');
            $('[data-sort]').each(function () {
                const id = $(this).prop('id');
                if (id != target) {
                    elementClassRemove(id, 'data-sort');
                }
            });
        });
    });
    ...
    $(document).on('doc.ready', function () {
        $('[data-sort="unsort"]').click(function () {
            $('[data-sort]').each(function () {
                ...
                elementClassRemove(target, 'data-sort');

                $('#' + target).children().each(function () {
                    if ($(this).prop('id')) {
                        const child = $(this).prop('id');
                        elementClassChange(child, 'fa-sort-amount-up', 'fa-sort-amount-down');
                    }
                });
            });
            $_SORT_BY = null;
...
function setVal(pElementId, pVal) {
    $(`#${pElementId}`).val(pVal);
    fieldReset(pElementId);
}

/* Cambia el valor de un elemento (Disabled) */
function setValDisabled(pElementId, pVal) {
    $(`#${pElementId}`).val(pVal);
    fieldReset(pElementId);
    elementDisabledOnly(pElementId);
}

/* Elimina el valor de un elemento */
function valClear(pElementId) {
...
function hasValue(pElementId) {
    ...
    if (isSelect(pElementId)) {
        return (val(pElementId) != "-" && val(pElementId) != "") ? true : false;
    }
    return val(pElementId) != "" ? true : false;
}
...
/* Removes the text from an element */
function textClear(pElementId) {
    elementClear(pElementId);
}
...
function elementHide(pElementId, pElementClass = null) {
    if (pElementClass == null) {
        ...
        $("." + pElementClass).attr("style", "display: none !important");
    }
}

/* Makes an element visible */
function elementShow(pElementId, pElementClass = null) {
    if (pElementId != null && pElementClass == null) {
...
function elementTooltipOn(pElementId, pMessage, isToogle = false) {
...
function elementDataTooltip({selector, tooltip, dir = null}) {
    if (exists(selector)) {
        elementClassAdd(selector, 'add-tooltip');
...
function fieldReset(field) {
    ...
    if(isCheckbox(fieldObj.field)) {
        elementUnchecked(fieldObj.field);
    }
    ...
    elementTooltipOff(fieldObj.field);
}
...
function formReady(formId) {
    if (formId != null) {
        ...
        $(selector).each(function () {
            ...
            if (existsElementInArray($_FORM_TYPES_ARRAY, input)) {
                const id = this.id;
                if (id != null && id != '') {
...
function enableFields(fields) {
    if (fields != null) {
        if (!isArrayEmpty(fields)) {
            fields.forEach(f => {
                elementReadOnlyFalse(f);
                elementAvailable(f);
            });
        } else {
            elementReadOnlyFalse(fields);
            elementAvailable(fields);
        }
    }
}
...
function disableFields(fields) {
    if (fields != null) {
        if (!isArrayEmpty(fields)) {
            fields.forEach(f => {
                elementReadOnlyTrue(f);
                elementDisabled(f);
            });
        } else {
            elementReadOnlyTrue(fields);
            elementDisabled(fields);
        }
    }
}
...
function inputGroupClassReset(groupId, field1, field2) {
    ...
    elementTooltipOff(field1);
    elementTagHide(field2);
    elementTooltipOff(field2);
}
...
function selectPickerVal(pIdSelect, pVal) {
    $('#'+pIdSelect).selectpicker('val', pVal);
}
...
function elementDisabled(pElementId) {
    $(`#${pElementId}`).val("");
    $(`#${pElementId}`).prop("disabled", true);
}

/* Habilita un elemento */
function elementAvailable(pElementId) {
    $(`#${pElementId}`).prop("disabled", false);
    elementReadOnlyFalse(pElementId);
}
	...
	$.fn.btnStatus = function(options) {
		...
		const btn_status = function ($btn) {
			...
			if(exists($btnId)) {
				...
				elementClassRemoveAll($divId);
				elementClassAdd($divId, $settings.animateOutClass);
				...
				setTimeout(function() {
					elementClassRemove($divId, $settings.animateOutClass);
					elementClear($divId);
					$('#'+$divId).append($html);
					elementClassAdd($divId, $settings.animateInClass);
 ...
 * @param pElementId Element id
 */
const elementChecked = (pElementId) => {
 ...
 * @param pElementId Element id
 */
const elementUnchecked = (pElementId) => {
...
function errorClass(pElementId) {
	if(exists(pElementId)) {
		if (jsOptions.bootstrapVersion === 4 || jsOptions.bootstrapVersion === 5) {
	        elementClassAdd(pElementId, 'is-invalid');
	    } else if (jsOptions.bootstrapVersion === 3) {
...
function errorClassRemove(pElementId) {
	if(exists(pElementId)) {
	    if (jsOptions.bootstrapVersion === 4 || jsOptions.bootstrapVersion === 5) {
	        elementClassRemove(pElementId, 'is-invalid');
	    } else if (jsOptions.bootstrapVersion === 3) {
...
function smallErrorClass(pElementId) {
	if(exists(pElementId)) {
	    if (jsOptions.bootstrapVersion === 4 || jsOptions.bootstrapVersion === 5) {
	        elementClassAdd(pElementId, 'invalid-feedback');
	    } else if (jsOptions.bootstrapVersion === 3) {
	        elementClassAdd(pElementId, 'form-message light');
	    }
    }
}
...
function smallErrorClassRemove(pElementId) {
	if(exists(pElementId)) {
	    if (jsOptions.bootstrapVersion === 4 || jsOptions.bootstrapVersion === 5) {
	        elementClassRemove(pElementId, 'invalid-feedback');
	    } else if (jsOptions.bootstrapVersion === 3) {
	        elementClassRemove(pElementId, 'form-message light');
	    }
    }
}
...
function elementTooltipError(pElementId, pTooltipMessage) {
    elementError(pElementId);
    elementTooltipOn(pElementId, pTooltipMessage);
}
...
function checkPaginateSize({selector, paginateSize}) {
    if (selector != null && paginateSize != null) {
        ...
        elementClassAdd(liPage, 'text-muted');
        ...
        for (let [key, value] of $_LI_PAGE_SIZE_MAP) {
            if (key === paginateSize && liPage.contains(value)) {
                ...
                elementClassAdd(item, 'text-muted');
...
function checkPaged(liPaginate, liName, paginateSize, field) {
    if (paginateSize != null) {
        elementClassAdd(liName, 'text-muted');
